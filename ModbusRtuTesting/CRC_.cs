    ...
    namespace CRT_COM.CRC
    {
        public class CRC_
        {
            public static string CRC(string data)
            {
                ...
                byte[] byteStr = Str_change_byte(data);
                ushort crc1 = CRC_GetModbus16(byteStr, byteStr.Length);
                string crc_ = Convert.ToString(crc1, 16).PadLeft(4, '0').ToUpper();
                return crc_;
            }
            public static ushort CRC_GetModbus16(byte[] pData, int nLength)
            {
                ushort cRc_16 = 0x0000;
                byte temp;
